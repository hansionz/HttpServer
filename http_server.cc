...
int HttpServer::Start(const std::string& ip, uint32_t port)
{
    // 1. 创建套接字
    ...
    // setsockopt函数相当于取消TIME_WAIT状态,让服务器可以尽快的重用连接
    int opt = 1;
...
int HttpServer::Process404(Context* context)
{
    ...
    resp->body = "<head><meta http-equiv=\"content-type\""
                 "content=\"text/html;charset=utf-8\"></head><h1>404!您的页面被喵星人偷走了!</h1>";
...
int HttpServer::ReadOneRequest(Context* context)
{
    ...
    //3.解析url,获取到 url_path 和 query_string
    ...
    //4.循环的按行读取数据，每读取到一行数据，就进行一次header的解析.读到空行说明header解析完毕
    ...
    //如果是POST请求,并且header中包含了content-length字段继续读取socket,获取body的内容
...
//解析一个标准的url比较复杂
//核心思路:
// 以?作为分割,从?左边来查找url_path,从?右边来查找query_string
...
// 如果使用split的话,可能有value中包含:这样就被切分成了多块
...
// 实现序列化把Response对象转换成一个string,写回到socket中
...
{
    *file_path = "./Html" + url_path;
    //判定一个路径是普通文件还是目录文件
    //1.linux的stat函数,可以查看文件类型
    //2.通过boost filesystem模块来进行判定
    //3.如果当前文件是一个目录,就可以进行一个文件名拼接,拼接上index.html
...
int HttpServer::ProcessCGI(Context* context)
{
    ...
    if(ret > 0)
    {
        ...
        close(child_read);
        close(child_write);
        //如果是POST请求,父进程就要把body写入到管道中
        if(req.method == "POST")
        {
            write(father_write, req.body.c_str(),req.body.size());
        }
        //阻塞式的读取管道,尝试把子进程的结果读取出来,并且放到 Response对象中
    ...
    else
    {
        ...
        dup2(child_read,0);
        dup2(child_write,1);
        //先获取到要替换的可执行文件是哪个（通过url_path来获取）
        std::string file_path;
        GetFilePath(req.url_path, &file_path);
        //进行进程的程序替换
        execl(file_path.c_str(), file_path.c_str(),NULL);
       //有我们的CGI可执行程序完成动态页面的计算,并且写回数据到管道
       //这部分逻辑,我们需要放到另外单独的文件中实现，并且根据该文件,编译生成我们的CGI可执行程序
    }
END:
    close(father_read);
    close(father_write);
